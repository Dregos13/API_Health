...
def hello():
    ...
    usuario = pandas.DataFrame(list(usuario.find({"_id": objInstance})))

    archivo = pandas.DataFrame(list(archivo.find()))
    ...
    mydict = {"userId": objInstance, "media": media, "positivo": enfermedad, "negativo": noEnfermedad}

    records.insert_one(mydict)

    return jsonify({'prob': media, 'attack case': enfermedad, 'notAttack case': noEnfermedad, 'success': True})
